...
/*
	В функции "affectStyleAround" вставил:
		if(!selectedTag.parent().is("."+vars.css+"_editor") && selectedTag.parent().is("[style]"))
	чтобы в теги внутри текстового поля не вставлялась строка стиля этого поля (в данном случае height)
	Не знаю на что это влияет, но пка все работает))
*/
...
(function($){		
	$.fn.jqte = function(options){
		...
		addParams('full','justifyFull','','',false); // justify full --> no hotkey
